...
// query for category
$categories = $cache->get_item($cache_key_category); // category for cache
if(!$categories){
    ...
    $cache->set_item($cache_key_category); // create category cache;
} // ***************************** end category *****************************************
        ...
        if($result){
            ...
            if($status == 'yes'){
                ...
                if($num_rows > 0){
                    ...
                    $cache->set_item($issue, $publicationdate);
                }
                ...
                if($num_rows > 0){
                    $cache->set_item($rowMaxDate, 'max_publication_date'); // create max publication date cache;
                }
                ...
                if($resultRss->num_rows > 0){
                    ...
                    $cache->set_item($posts, $cache_key_rss); // create cache;
                }
            }
...
if($currentPage == 'issueEdit.php'){
    ...
    if(array_key_exists('submit', $_POST)){
        ...
        if($resultIssueUpdate){
            ...
            if($num_rows > 0){
                $issue = $result->fetch_assoc();
                $cache->set_item($issue, 'iss_' . $publicationdate); // create issue cache;
            }
            ...
            if($num_rows > 0){
                ...
                $cache->set_item($pages, $cache_key_daily_page); // create daily_pages cache;
            }
    ...
    if(array_key_exists('submit', $_POST)){
        ...
        if($resultArticleInsert){
			...
			if($submit == 'published') {
				...
				$cache->set_item($latest_news, $cache_key_latest_news); // create cache;
				...
				if($block == 1 && $frontpage == 'yes'){
					...
					$cache->set_item($home_lead, 'home_lead');
				}
				...
				if($block == 2 && $frontpage == 'yes'){
					...
					$cache->set_item($second_lead, 'second_lead'); // create cache;
				}
				...
				if($block == 3 && $frontpage == 'yes'){
					...
					$cache->set_item($feature_lead, 'feature_lead'); // create cache;
				}
				...
				if($block == 4 && $frontpage == 'yes' && $section == 1){
					...
					$cache->set_item($bangladesh_lead, 'bangladesh_lead'); // create cache;
				}
				...
				if($block == 0 && $frontpage == 'yes' && $section == 1){
					...
					$cache->set_item($bangladesh_others, 'bangladesh_others'); // create cache;
				}
				...
				if($block == 0 && $frontpage == 'yes' && $section == 2){
					...
					$cache->set_item($international, 'international_home'); // create cache;
				}
				...
				if($block == 0 && $frontpage == 'yes' && $section == 3){
					...
					$cache->set_item($sports, 'sports_home'); // create cache;
				}
				...
				if($block == 4 && $frontpage == 'yes' && $section == 11){
					...
					$cache->set_item($life_style_lead, 'life_style_lead'); // create cache;
				}
				...
				if($block == 0 && $frontpage == 'yes' && $section == 11){
					...
					$cache->set_item($life_style_others, 'life_style_others'); // create cache;
				}
				...
				if($block == 0 && $frontpage == 'yes' && $section == 9){
					...
					$cache->set_item($feature, 'feature_home'); // create cache;
				}
				...
				if($block == 0 && $frontpage == 'yes' && $section == 10){
					...
					$cache->set_item($science_technology, 'science_technology_home'); // create cache;
				}
				...
				if($block == 0 && $frontpage == 'yes' && $section == 6){
					...
					$cache->set_item($opinion, 'opinion_home'); // create cache;
				}
                ...
                if($section_category){
                    $cache->delete_item($section_category);
                    $cache->delete_item($section_category_count);
...
//        echo '<pre>';
//        echo 'Post from cache: <br />';
//        var_dump($post, $cache_key_post);
//        echo '</pre>';
        // query for publication date
    ...
    if(array_key_exists('submit', $_POST)){
        ...
        foreach ($_POST as $key => $value){
            if($key == 'submit'){
                ...
                if(!is_array($value)) ${$key} = mysqli_real_escape_string($connectionWrite, trim($value));
            }
        }
...
//        exit();
        if($resultArticaleUpdate){
			...
			$cache->set_item($post); // create cache;
			if($submit == 'published' || $row['news_status'] == 'published'){
				...
				if(($block == 1 && $frontpage == 'yes') || $row['block_id'] == 1){
					...
					// create cache
					while($rows = $resultHomeLead->fetch_assoc()){$home_lead[] = $rows;}
					$cache->set_item($home_lead, 'home_lead'); // create cache;
				}
