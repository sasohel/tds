...
if(array_key_exists('section', $_GET)){ // get section name
	...
	$category['alias'] = !is_numeric($alias[1]) ? $alias[1] : false;
	$page_no = $alias[2] ? floor($alias[2]) : (is_numeric($alias[1]) ? floor($alias[1]) : $page_no);
...
if($post_uri['id']) {
	...
	if($post_status == 'published'){ // when post is published
		if($post['issue_date'] && ($post['issue_date'] != '0000-00-00')){ // issue
			...
			$issue = get_cache($cache_key_issue, $cache_dir_issue); // get issue from cache
			if($_SERVER['REMOTE_ADDR'] == '192.95.39.46'){
                            var_dump($post['issue_date']);
                        }
			if(!$issue){
				$sql = "SELECT * FROM publicationdate WHERE publication_date = '" . $cache_key_issue . "'";
				$connection = !isset($connection) ? dbConnect('read', MYDATABASE) : $connection;
				$result = $connection->query($sql) or die(mysqli_error($connection));
...
$max_publication_date = get_cache('max_publication_date', $cache_dir_issue); // get max publication date from cache
...
// Clear cache of ‘আজকের পত্রিকা’s sub menu & daily pages after 3:00 AM
$datetime_3am = strtotime(date('d F Y', strtotime('now')) . ' 3:00:00 AM');
$datetime_310am = strtotime(date('d F Y', strtotime('now')) . ' 3:00:10 AM');
$now_datetime = strtotime('now');

if($now_datetime > $datetime_3am && $now_datetime < $datetime_310am){
    $cache = new Cache();
    $cache_file = $cache->setCachePath($cache_dir_page . $date_line['date'] . '/');
    $cache_data = $cache->retrieveAll(true);
    $cached_at = strtotime(date("F d Y h:i:s A", $cache_data['last-page']['time']));

    if($cached_at < $datetime_3am){
        $cache->eraseAll();
        $cache->setCachePath($cache_dir_daily_page);
        $cache->erase($date_line['date']);
    }
}
...
if(array_key_exists('page', $_GET) || $issue_date){ // get page info
	...
	if(!$publication_date['date'] && !$issue_date){
        ...
        $issue = get_cache($publication_date['date'], $cache_dir_issue); // get issue from cache
        if($issue){
	...
	if($cache_key_daily_page){
		$daily_pages = get_cache($cache_key_daily_page, $cache_dir_daily_page); // daily page for cache
                if($_SERVER['REMOTE_ADDR'] == '103.10.197.59'){
                    echo '<pre>';
                    var_dump($cache_key_daily_page, $daily_pages);
                    echo '</pre>';
                }
		if(!$daily_pages){
...
if($section['id'] && !$post_uri['id']){
	...
	$print_date = $issue_date ? $issue_date : $publication_date['date'];
//	var_dump($print_date, $issue_date, $publication_date['date']);
    ...
    if($is_mobile){
        ...
        if($_SERVER['REMOTE_ADDR'] == '103.10.197.131'){
            var_dump($post_uri['id']);
        }
	$main_sub_menu .= '<ul class="subMenu">';
	$breadcrumbs .= '<li class="bcrmSeparator">';
	$breadcrumbs .= '<span class="bcrmSeparatorInner">›</span>';
...
if($currentPage == 'index.php'){
    ...
    // Latest news
	$latest_news = get_cache($cache_key_latest_news, $cache_dir_latest_news); // get last update from cache
	if(!$latest_news){
		$this_time = date('Y-m-d H:i:s');
	...
	if(empty($_GET)){
		// home lead
		$home_lead = get_cache('home_lead', $cache_dir_home); // get home lead from cache
		...
		// second lead
		$second_lead = get_cache('second_lead', $cache_dir_home); // get home second lead cache
		if(!$second_lead){
			$sqlSecondLead = "
				SELECT
				news.news_id AS news_id,
		...
		// Feature lead
		$feature_lead = get_cache('feature_lead', $cache_dir_home); // get home feature lead cache
		if(!$feature_lead){
			$sqlFeatureLead = "
				SELECT
				news.news_id AS news_id,
		...
		// Bangladesh lead
		$bangladesh_lead = get_cache('bangladesh_lead', $cache_dir_home); // get home Bangladesh lead cache
		if(!$bangladesh_lead){
			$sqlBangladeshLead = "
				SELECT
				news.news_id AS news_id,
		...
		// Bangladesh others
		$bangladesh_others = get_cache('bangladesh_others', $cache_dir_home); // get home bangladesh others cache
		...
		// International
		$international = get_cache('international', $cache_dir_home); // get home international cache
		if(!$international){
			$sqlInternational = "
				SELECT
				news.news_id AS news_id,
		...
		// sports
		$sports = get_cache('sports', $cache_dir_home); // get home sports cache
		if(!$sports){
			$sqlSports = "
				SELECT
				news.news_id AS news_id,
		...
		// Life style lead
		$life_style_lead = get_cache('life_style_lead', $cache_dir_home); // get home life style lead cache
		if(!$life_style_lead){
			$sqlLifeStyleLead = "
				SELECT
				news.news_id AS news_id,
		...
		// Life style others
		$life_style_others = get_cache('life_style_others', $cache_dir_home); // get home life style others cache
		...
		// feature
		$feature = get_cache('feature', $cache_dir_home); // get home feature cache
		if(!$feature){
			$sqlFeature = "
				SELECT
				news.news_id AS news_id,
		...
		// science-technology
		$science_technology = get_cache('science_technology', $cache_dir_home); // get home science-technology cache
		...
		// opinion
		$opinion = get_cache('opinion', $cache_dir_home); // get home opinion cache
		if(!$opinion){
			$sqlOpinion = "
				SELECT
				news.news_id AS news_id,
    ...
    if ($section['alias']) {
        ...
        $number_of_section = get_cache($cache_key_section_page_count, $cache_dir_section_page); // get cache
        if (!$number_of_section) {
            ...
            create_cache($cache_key_section_page_count, $number_of_section, $cache_dir_section_page); // create cache;
        }
        ...
        if($page_no > 1){
            ...
            $posts = get_cache($cache_key_section_page, $cache_dir_section_page); // get cache
            if (!$posts) {
                $sqlNewsFront = "SELECT
            news.news_id AS news_id,
            head_line,
            ...
            ORDER BY publication_date DESC LIMIT $record_start, $item_per_page";
                ...
                create_cache($cache_key_section_page, $posts, $cache_dir_section_page); // create cache;
            }
        }
    } elseif (!($page['alias'])) {
...
//        $cache_key_page = $page['alias'] . '_' . $publication_date['date'];
        $cache_key_page = $page['alias'];
        $posts = get_cache($cache_key_page, $cache_dir_page); // get cache
        ...
        if (!$posts) {
            ...
            create_cache($cache_key_page, $posts, $cache_dir_page); // create cache;
        }
...
if($currentPage == 'sangram_rss.php'){
    $posts = get_cache($cache_key_rss, $cache_dir_rss); // cache
    if(!$posts) {
